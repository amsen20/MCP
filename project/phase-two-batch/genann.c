...
void genann_copy_nomem(genann *dst, genann const *src) {
    const int size = sizeof(genann) + sizeof(double) * (src->total_weights + src->total_neurons + (src->total_neurons - src->inputs));
    memcpy(dst, src, size);
...
void genann_clear(genann *ann) {
    memset(ann->weight, 0, sizeof(double) * ann->total_weights);
}
