...
double const *genann_run_parallel(genann const *ann, double const *inputs) {
    ...
    if (!ann->hidden_layers) { // never gets here
        double *ret = o;
...
void genann_train_parallel(genann const *ann, double const *inputs, double const *desired_outputs, double learning_rate) {
    ...
    /* Train the outputs. */
    {
        ...
        #pragma omp parallel for private(j)
        for (j = 0; j < ann->outputs; ++j) {
            double *cur = w + j * (1 + step);
            *cur += *(d + j) * learning_rate * -1.0;
            for (k = 1; k < step + 1; ++k) {
                *(cur + k) += *(d + j) * learning_rate * i[k-1];
            }
        }
